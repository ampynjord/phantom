    ...
    def next_steps(self, state: ExecutionState) -> list[DecisionStep]:
        ...
        if "common_paths" not in executed and has_http:
            queue.append(DecisionStep(module="common_paths", priority=55,
                ...
                source="heuristic"))
        if "os_audit" not in executed:
            queue.append(DecisionStep(module="os_audit", priority=50,
                reason="Run local OS security audit (Lynis/PowerShell) if target is localhost.",
                source="heuristic"))
